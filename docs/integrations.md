...
→ Arpio** imports your protected resources as component proposals. Create a
**read-only** API key in Arpio and paste it into the form (with your Arpio
account ID if asked). The key is used for that request only — it is not
written to disk. Imported components arrive pre-annotated with replication
information where Arpio provides it.
